...
def generate_repro(*, workspace: Path | str, crash_report: Path | str, profile: str = "raw") -> dict[str, Any]:
...
set -euo pipefail
cd "$(dirname "$0")/.."
exec scripts/run_target.sh --profile {profile} < {json.dumps(crash_input)}
"""
    ...
    append_event(
        ...
        summary=f"Generated replay script for {crash_id} using profile {profile}",
